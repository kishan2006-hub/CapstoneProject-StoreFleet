...
export const getAllProducts = async (req, res, next) => {
  try {
    ...
    const products = await findProductByDetails(req.query, resultPerPage);
...
export const deleteReview = async (req, res, next) => {
  try {
    ...
    if (!productId || !reviewId) {
      return next(
        new ErrorHandler(400, "Please provide productId and reviewId")
      );
    }
    ...
    if (reviewIndex < 0) {
      return next(new ErrorHandler(400, "Review does not exist"));
    }

    const reviewToDelete = reviews[reviewIndex];

    // User Check — only review owner OR admin can delete
    if (
      req.user._id.toString() !== reviewToDelete.user.toString() &&
      req.user.role !== "admin"
    ) {
      return next(
        new ErrorHandler(403, "Only review owner or admin can delete review")
      );
    }
